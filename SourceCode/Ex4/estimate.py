...
# --- Config ---
DATA_PATH = "Report/OUTPUT_BAI4/player_transfer_data.csv"
...
ID_COLS = ["Player"] # Removed Nation, Team, Team_TransferSite as they might not be in the scraped data
CAT_COLS = ["Position", "Team"] # Included 'Team' as a categorical feature
...
def load_data(data_path, target_col):
    try:
        ...
        if target_col not in df.columns:
            print(f"Error: Target column '{target_col}' not found in '{data_path}'.")
            return None
...
def evaluate(model, X_test, y_test, feat_names):
    ...
    plt.xlabel("Actual Transfer Value (M EUR)")
    plt.ylabel("Predicted Transfer Value (M EUR)")
    plt.title("Predicted vs. Actual Transfer Value")
    plt.tight_layout()
    ...
    plt.xlabel('Predicted Transfer Value (M EUR)')
...
def main():
    data = load_data(DATA_PATH, TARGET)
    if data is None:
        return
